...
Route::get('/api/products', function (Request $request) {
    ...
    $client = new Rest($session->getShop(), $session->getAccessToken());
    $response = $client->get('products', query: ['fields' => 'id, title, body_html, image, variants', 'limit' => 10]);

    return response($response->getDecodedBody());
})->middleware('shopify.auth');

Route::put('/api/products/{id}', function (int $id, Request $request) {
    /** @var AuthSession */
    $session = $request->get('shopifySession'); // Provided by the shopify.auth middleware, guaranteed to be active

    $client = new Rest($session->getShop(), $session->getAccessToken());
    $response = $client->put("products/$id", body: ['product' => $request->json()->all()]);
